...
root.title("Bypass iPhone 7/7+/8/8+/X")
...
""")
    ...
    message = "These are the tools for bypassing the iPhone 7/7+/8/8+/X..."
    tk.Label(win, text="These are the tools for bypassing the iPhone 7/7+/8/8+/X...").pack()
...
def purplemodebypass():
    response = messagebox.askokcancel("Info", """This method ONLY supports the iPhone 7.
...
choosebypass = tk.Label(frame, text="Choose a bypass for the iPhone 7, 7+, 8, 8+ or X...", font=("helvetica", 20))
choosebypass.place(x=70, y=14)
...
purplemodebypassbutton = tk.Button(frame, text="Purple Mode Unlock", width="18", command=purplemodebypass)
purplemodebypassbutton.place(x=180, y=170)
